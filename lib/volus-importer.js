...
//    (descoberto via Inspetor de JSON 2026-06-02 · atualizado 2026-06-09)
//
//    fuel.product[].total_value   → REAIS (raw · API mudou 09/06)
//    maint.products[].total_price → REAIS (raw)
//    fuel.unit.name               → 'FMAS'
//    maint.unit.description       → 'ADMINISTRACAO'
//
//    HISTÓRICO: até 02/06 a API de fuel mandava em CENTAVOS (total_value
//    "46087.0438" para 86L diesel = R$ 460.87). Em 09/06 detectamos que
//    Vólus mudou pra REAIS (qty × unit_price = total_value confirmou em
//    9 amostras de 3 empresas). Helpers agora tratam como raw.
// ---------------------------------------------------------------------
const nz = v => (+v || 0);

/** Total agregado de combustível em REAIS (raw — Vólus 2026-06-09 em diante) */
export function fuelTotalBRL(r) {
  return (r.product || []).reduce((s, p) => s + nz(p?.total_value), 0);
}

/** Total agregado de manutenção em REAIS (raw — sem dividir) */
export function maintTotalBRL(r) {
  ...
  if (tp > 0) return tp;
  // Legacy fallbacks (campos alternativos no produto)
  for (const f of ['total_value','cost','value','price','amount','gross_value','net_value','total']) {
    const sum = (r.products || []).reduce((s, p) => s + nz(p?.[f]), 0);
    if (sum > 0) return sum;
  }
  // Total no nível da transação
  if (r.total_price != null) return nz(r.total_price);
  if (r.total_value != null) return nz(r.total_value);
  if (r.amount      != null) return nz(r.amount);
  // Em invoice
  if (r.invoice?.total_price != null) return nz(r.invoice.total_price);
  if (r.invoice?.total_value != null) return nz(r.invoice.total_value);
...
/** Empenho da manutenção · suporta array (Vólus 2026-06-09+) e scalar (legacy)
 *  Quando há múltiplos empenhos, retorna todos joined por " · "
 */
export function maintCommitment(r) {
  // NOVO (2026-06-09+): purchase_orders[] (array plural)
  if (Array.isArray(r.purchase_orders) && r.purchase_orders.length) {
    const nums = r.purchase_orders.map(po => po?.commitment_number).filter(Boolean);
    if (nums.length) return nums.join(' · ');
  }
  // LEGACY: purchase_order (scalar)
...
/** Processo da manutenção · suporta array (2026-06-09+) e scalar (legacy) */
export function maintProcess(r) {
  if (Array.isArray(r.purchase_orders) && r.purchase_orders.length) {
    const procs = r.purchase_orders.map(po => po?.process_number).filter(Boolean);
    if (procs.length) return [...new Set(procs)].join(' · '); // dedup (geralmente repetido)
  }
...
/** Lista crua dos empenhos da manutenção (array de números) · útil pra DB schema 1:N */
export function maintCommitmentList(r) {
  if (Array.isArray(r.purchase_orders) && r.purchase_orders.length) {
    return r.purchase_orders.map(po => po?.commitment_number).filter(Boolean);
  }
  const single = r.purchase_order?.commitment_number || r.commitment_number;
...
export function normalizeFuelRow(r, company) {
  ...
  return {
    ...
    product_unit_price_brl:       p0.unit_price != null ? nz(p0.unit_price) : null,
    product_total_value_brl:      p0.total_value != null ? nz(p0.total_value) : null,
...
/** Itens individuais de combustível (linhas separadas · valores em REAIS raw) */
export function normalizeFuelItems(r) {
  return (r.product || []).map((p, i) => ({
    ...
    unit_price_brl:    nz(p.unit_price),
    total_value_brl:   nz(p.total_value),
  }));
}
